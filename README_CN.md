...
6. **修复Python运行时错误**：在uClibc-ng中启用堆栈保护（SSP）支持（`BR2_TOOLCHAIN_BUILDROOT_USE_SSP=y`），解决了`symbol '__stack_chk_guard': can't resolve symbol`错误。详见[PYTHON_SSP_FIX.md](PYTHON_SSP_FIX.md)。
## SDK 使用说明
